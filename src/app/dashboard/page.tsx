        ...
        <div className="lg:col-span-4 space-y-6">
          <Card className="glass-panel border-white/10 rounded-[2rem] overflow-hidden">
            ...
            <CardContent className="px-8 -mt-12 pb-8 text-center space-y-4">
              ...
              <div>
                <h2 className="text-2xl font-headline font-bold uppercase italic">{user.name}</h2>
          ...
          <nav className="glass-panel border-white/10 rounded-[2rem] p-4 space-y-2">
            ...
            <Button variant="ghost" className="w-full justify-start text-red-500 hover:bg-red-500/10 hover:text-red-400 p-4 rounded-2xl mt-4">
        ...
        <div className="lg:col-span-8 space-y-8">
          ...
          <Tabs defaultValue="orders" className="w-full">
            ...
            <TabsContent value="orders" className="space-y-4">
              {[
                { id: "ORD-9901", date: "24 Feb, 2024", item: "PS Plus 12-Month", price: "Rs. 15,958", status: "Delivered" },
                { id: "ORD-9842", date: "12 Jan, 2024", item: "Steam Wallet $50", price: "Rs. 6,650", status: "Processing" },
                { id: "ORD-9711", date: "28 Dec, 2023", item: "Xbox Game Pass", price: "Rs. 1,995", status: "Delivered" },
              ].map((order, i) => (
                <div key={i} className="glass-panel p-6 rounded-3xl border-white/5 flex flex-col md:flex-row md:items-center justify-between gap-6 hover:border-primary/30 transition-all group">
                  <div className="flex items-center gap-6">
                    ...
                    <div>
                      <div className="text-xs font-bold text-muted-foreground uppercase tracking-widest">{order.date} • {order.id}</div>
                      <h4 className="text-lg font-bold font-headline uppercase">{order.item}</h4>
                      <p className="text-sm font-bold text-white mt-1">{order.price}</p>
                    </div>
                  </div>
                  <div className="flex items-center justify-between md:justify-end gap-6">
                    <Badge className={order.status === 'Delivered' ? 'bg-green-500/10 text-green-500 border-green-500/20' : 'bg-blue-500/10 text-blue-500 border-blue-500/20'}>
                      {order.status}
                    </Badge>
            ...
            <TabsContent value="wishlist">
              <div className="text-center py-20 bg-white/5 rounded-3xl border border-dashed border-white/10">
                ...
                <Button className="mt-8 bg-primary hover:bg-primary/90 rounded-xl px-8 font-bold">START SHOPPING</Button>
              </div>
            </TabsContent>
          </Tabs>
        </div>
